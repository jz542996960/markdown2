...
​     在setValue中 加入了while 循环，虽然trd2 setValue 后 notify了。但是因为内部有循环，没有释放key的锁，那么导致trd1 ，虽然被唤醒 但是一直无法获取到锁，也就没法继续执行。去掉while 循环后，线程二 notify后，线程1被唤醒并且获取到了锁，wait唤醒后，继续走下一次循环判断条件为true，会打印value change。 
